...
class TradingScreen(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.main_split.addWidget(left_card)
        ...
        tape_scroll = QScrollArea(); tape_scroll.setWidgetResizable(True); tape_scroll.setFrameShape(QFrame.NoFrame); tape_scroll.setWidget(self.tape_card)
        order_scroll = QScrollArea(); order_scroll.setWidgetResizable(True); order_scroll.setFrameShape(QFrame.NoFrame); order_scroll.setWidget(self.order_card)
        self.right_split.addWidget(tape_scroll)
        self.right_split.addWidget(order_scroll)
        self.right_split.setStretchFactor(0, 55)
        self.right_split.setStretchFactor(1, 45)

        self.main_split.addWidget(self.right_split)
        self.main_split.setStretchFactor(0, 70)
        self.main_split.setStretchFactor(1, 30)

        root.addWidget(self.main_split, 7)

        # Compact/right-tabs mode for narrow windows
        self.right_tabbar = QTabBar()
        self.right_tabbar.addTab("Tape")
        self.right_tabbar.addTab("Order")
        self.right_tabbar.addTab("Position")
        self.right_tabbar.currentChanged.connect(self._on_right_tab_changed)
        self.right_tabbar.hide()

        self.right_stack = QStackedWidget()
        self.right_stack.hide()
        self.right_stack.addWidget(tape_scroll)
        self.right_stack.addWidget(order_scroll)

        self.position_card = self._card()
        position_layout = QVBoxLayout(self.position_card)
        position_layout.setContentsMargins(16, 16, 16, 16)
        position_layout.setSpacing(8)
        self.position_card_compact = QLabel("Qty: -\nAvg: -\nCurrent: -\nUPnL: -\nPnL%: -")
        self.position_card_compact.setObjectName("secondaryLabel")
        position_layout.addWidget(QLabel("Position"))
        position_layout.addWidget(self.position_card_compact)
        self.right_stack.addWidget(self.position_card)

        root.addWidget(self.right_tabbar)
        root.addWidget(self.right_stack, 5)
        ...
        self.position_card_label = QLabel("Qty: -\nAvg: -\nCurrent: -\nUPnL: -\nPnL%: -")
        self.position_card_label.setObjectName("secondaryLabel")
        bottom.addWidget(self._boxed("Account", self.account_card), 0, 0)
        bottom.addWidget(self._boxed("Position", self.position_card_label), 0, 1)
    ...
    def _apply_responsive_mode(self) -> None:
        ...
        self.main_split.setVisible(not compact)
        self.right_tabbar.setVisible(compact)
        self.right_stack.setVisible(compact)
    ...
    def _on_worker_success(self, key: str, payload: object) -> None:
        if key == "quote":
            ...
            if q.change_pct > 0:
                self.price_label.setStyleSheet("color:#E5484D;")
            elif q.change_pct < 0:
                self.price_label.setStyleSheet("color:#3178F6;")
            else:
                self.price_label.setStyleSheet("color:#8B95A1;")
    ...
    def _create_condition(self) -> None:
        ...
        try:
            ...
            show_toast(self, "Condition order created", "success")
            self._refresh_condition_tables()
        except Exception as exc:
    ...
    def _render_account(self, account, positions) -> None:
        if account is None:
            ...
            txt = "Qty: -\nAvg: -\nCurrent: -\nUPnL: -\nPnL%: -"
            self.position_card_label.setText(txt)
            self.position_card_compact.setText(txt)
            return
    ...
    def _fail_safe(self, reason: str) -> None:
        ...
        show_toast(self, "Emergency stop engaged", "error")
        if self.alert:
            self.alert(f"🚨 {reason}")
